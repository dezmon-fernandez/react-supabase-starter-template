...
# prose (CLAUDE.md anywhere, a documentation standard, a plan, a feature idea, a skill,
# a command), emits the JSON that injects the authoring-for-agents smell-test reminder
# into the editing session's context. Any other path: silent, exit 0 — the gate must
# never make noise on code edits and must never fail loudly (a malformed payload yields
# an empty path, which simply doesn't match).

changed_path=$(jq -r '.tool_input.file_path // empty' 2>/dev/null)

case "$changed_path" in
  */CLAUDE.md | */.agents/documentation/* | */.agents/plans/* | */.agents/feature-ideas/* | */.claude/skills/* | */.claude/commands/*)
    jq -n '{
      hookSpecificOutput: {
